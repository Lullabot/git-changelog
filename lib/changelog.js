...
class Changelog {
    ...
    /**
     * Run a JQL query to find multiple issues and retrieve any specific field data.
     */
    async jiraSearch(jql) {
        const defaultFields = ['key', 'summary', 'issuetype'];
        const customFields = this.getOption('jiraFields') || [];

        const results = await this.jiraApi.searchJira(jql, {
            maxResults: 100,
            fields: [...defaultFields, ...customFields]
        }).catch(err => {
            err.message = "Error executing JQL search.\n" + err?.message;
            throw err;
        });

        const issues = [];
        results.issues.forEach(issue => {
            issues.push(this.formatJiraIssue(issue));
        });
    ...
    async getJiraIssue(issueKey) {
        ...
        // Format the Jira issue consistently.
        const issueDetails = this.formatJiraIssue(jiraIssue);
    ...
    /**
     * Consistently format the Jira issue.
     *
     * @param jiraIssue
     * @returns {{issueNumber, title: string, type, url: string}}
     */
    formatJiraIssue(jiraIssue) {
        // Build base issue details object.
        const issueDetails = {
            issueNumber: jiraIssue.key,
            title: jiraIssue.fields.summary,
            type: jiraIssue.fields.issuetype.name,
            url: `${this.jiraApi.protocol}//${this.jiraApi.host}/browse/${jiraIssue.key}`,
        };
